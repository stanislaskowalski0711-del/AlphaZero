...
def ecIterator(grammar, tasks,
    ...
    if testingTimeout is not None and testingTimeout > 0 and len(testingTasks) == 0:
    ...
    eprint("Running EC%s on %s @ %s with %d CPUs and parameters:" %
           (message, hostname, datetime.datetime.now(), CPUs))
    ...
    for j in range(resume or 0, iterations):
        ...
        if testingTimeout is not None and testingTimeout > 0 and ((j % testEvery == 0) or (j == iterations - 1)):
        ...
        # Sleep-G disabled - using only Sleep-R (recognition)
        eprint("Skipping consolidation (disabled).")
...
# consolidate function removed - Sleep-G disabled
