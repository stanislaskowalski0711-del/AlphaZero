...
class Frontier(object):
    ...
    def filterByExactZeroLikelihood(self):
        """只保留 logLikelihood 严格为 0 的程序"""
        self.entries = [
            e for e in self.entries if e.logLikelihood == 0.0]
        return self
    ...
    def topKByMDL(self, k):
        """只考虑 logLikelihood=0 的程序，按 MDL（logPrior）排序"""
        if k == 0: return Frontier([], self.task)
        if k < 0: return self
        
        # 只保留 logLikelihood 严格为 0 的程序
        valid_entries = [e for e in self.entries if e.logLikelihood == 0.0]
        
        # 按 logPrior 排序（MDL 越低越好，即 logPrior 越大越好）
        newEntries = sorted(valid_entries,
                            key=lambda e: (-e.logPrior, str(e.program)))
        return Frontier(newEntries[:k], self.task)
